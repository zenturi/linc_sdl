...
extern class SDL {
	...
	static inline function getError():String
		return cast _getError();
	...
	static inline function getRevision():String
		return cast _getRevision();
	...
	static function renderCopyEx(renderer:Renderer, texture:Texture, srcrect:SDLRect, dstrect:SDLRect, angle:Float, center:SDLPoint,
		flip:SDLRenderFlip):Int;
	...
	static inline function renderDrawLines(renderer:Renderer, points:Array<SDLPoint>):Void {
		if (points.length % 2 != 0)
			throw "renderDrawLines: points not divisible by 2!";
	...
	static inline function renderDrawPoints(renderer:Renderer, points:Array<SDLPoint>):Void {
		for (p in points)
			renderDrawPoint(renderer, p.x, p.y);
	} // renderDrawPoints
	...
	static function updateYUVTexture(texture:Texture, rect:SDLRect, Yplane:BytesData, Ypitch:Int, Uplane:BytesData, Upitch:Int, Vplane:BytesData,
		Vpitch:Int):Int;
	// SDL_cpuinfo.h
	...
	static inline function getKeyFromName(name:String):SDLKeycode
		return _getKeyFromName(cast name);
	...
	static inline function getKeyName(key:SDLKeycode):String
		return cast _getKeyName(key);
	...
	static inline function getScancodeFromName(name:String):SDLScancode
		return _getScancodeFromName(cast name);
	@:native('SDL_GetScancodeName')
	private static function _getScancodeName(scan:SDLScancode):cpp.ConstCharStar;
	static inline function getScancodeName(scan:SDLScancode):String
		return cast _getScancodeName(scan);
	...
	static inline function gameControllerMapping(gamecontroller:GameController):String
		return _gameControllerMapping(gamecontroller).toString();
	...
	static inline function gameControllerName(gamecontroller:GameController):String
		return _gameControllerName(gamecontroller).toString();
	...
	static inline function gameControllerNameForIndex(joystick_index:Int):String
		return _gameControllerNameForIndex(joystick_index).toString();
	...
	// :todo:
	@:native('linc::sdl::joystickGetGUID')
	static function joystickGetGUID(joystick:Joystick):cpp.ConstCharStar;
	...
	static inline function joystickName(joystick:Joystick):String
		return _joystickName(joystick).toString();
	@:native('SDL_JoystickNameForIndex')
	private static function _joystickNameForIndex(device_index:Int):cpp.ConstCharStar;
	static inline function joystickNameForIndex(device_index:Int):String
		return _joystickNameForIndex(device_index).toString();
	...
	static inline function hapticName(device_index:Int):String
		return cast _hapticName(device_index);
	...
	static inline function getThreadName(thread:Thread):String
		return cast _getThreadName(thread);
	...
	static inline function getDisplayName(displayIndex:Int):String
		return cast _getDisplayName(displayIndex);
	...
	static function pushEvent(event:cpp.RawPointer<Event>):Int;
	@:native("SDL_PeepEvents")
	static function peepEvents(events:Event, numevents:Int, action:SDL.SDLEventAction, minType:Int, maxType:Int):Int;
	// SDL_system.h
	#if (ios || tvos)
	...
	static inline function androidGetExternalStoragePath():String
		return cast _androidGetExternalStoragePath();
	@:native('SDL_AndroidGetInternalStoragePath')
	private static function _androidGetInternalStoragePath():cpp.ConstCharStar;
	static inline function androidGetInternalStoragePath():String
		return cast _androidGetInternalStoragePath();
	...
	private static function init_event_watch(func:cpp.Callable<Int->cpp.RawPointer<sdl.Event>->Int>):Void;
...
private class SDL_helper {
	...
	static function del_event_watch(func:SDLEventFilter) {
		...
		if (_found == null)
			throw "Can't find event watch to remove, was it added?";
	...
	static function event_watch_callback(_watchid:Int, _event:cpp.RawPointer<sdl.Event>):Int {
...
typedef SDLEventFilter = Dynamic->cpp.RawPointer<sdl.Event>->Int;
...
@:enum
abstract SDLInitFlags(Int) from Int to Int {
...
@:enum
abstract SDLWindowPos(Int) from Int to Int {
...
#if android
@:enum
abstract SDLExternalStorageState(Int) from Int to Int {
...
@:enum
abstract SDLLogCategory(Int) from Int to Int {
...
@:enum
abstract SDLRendererFlags(Int) from Int to Int {
...
@:enum
abstract SDLBlendMode(Int) from Int to Int {
...
@:enum
abstract SDLTextureAccess(Int) from Int to Int {
...
@:enum
abstract SDLThreadPriority(Int) from Int to Int {
...
@:enum
abstract SDLRenderFlip(Int) from Int to Int {
...
@:enum
abstract SDLEventState(Int) from Int to Int {
	var SDL_QUERY = -1;
	var SDL_IGNORE = 0;
	var SDL_DISABLE = 0;
	var SDL_ENABLE = 1;
...
@:enum
abstract SDLGLAttr(Int) from Int to Int {
...
@:enum
abstract SDLGLprofile(Int) from Int to Int {
...
@:enum
abstract SDLGameControllerButton(Int) from Int to Int {
...
@:enum
abstract SDLGameControllerAxis(Int) from Int to Int {
...
@:enum
abstract SDLEventAction(Int) from Int to Int {
	var SDL_ADDEVENT = 0;
	var SDL_PEEKEVENT = 1;
	var SDL_GETEVENT = 2;
} // SDLEventAction
...
@:enum
abstract SDLMessageBoxFlags(Int) from Int to Int {
...
@:enum
abstract SDLKeymod(Int) from Int to Int {
...
@:enum
abstract SDLHatValue(Int) from Int to Int {
...
@:enum
abstract SDLWindowEventID(Int) from Int to Int {
...
@:enum
abstract SDLEventType(UInt) from UInt to UInt {
...
@:enum
abstract SDLSystemCursor(Int) from Int to Int {
...
@:enum
abstract SDLWindowFlags(Int) from Int to Int {
...
// extern class SDL_MouseWheelDirectionImpl {}

@:keep
@:include('linc_sdl.h')
@:enum extern abstract SDL_MouseWheelDirection(Int) to Int {
	@:native('SDL_MOUSEWHEEL_NORMAL') var SDL_MOUSEWHEEL_NORMAL;
